...
from schooldjango.authentication import CustomJWTAuthentication  
...
def toggle_recording(request, classe_id):
    ...
    async def _start():
        ...
        try:
            ...
            filename = f"audio_{classe.id}_{timestamp}.ogg"
            ...
            for p in participants.participants:
                for t in p.tracks:
                    if t.source == api.TrackSource.SCREEN_SHARE:
                        ...
                        screen_filename = f"screen_{classe.id}_{ts}.webm"
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def register(self, request):
        """Inscription publique - l'utilisateur crée son propre compte"""
        with transaction.atomic():
            role = request.data.get('role')
            
            # Seuls professeur et élève peuvent s'inscrire eux-mêmes
            if role not in ('professeur', 'eleve'):
                return Response(
                    {'error': 'Seuls les professeurs et élèves peuvent s\'inscrire.'}, 
                    status=400
                )
            ...
            display_name = request.data.get('display_name', '')
            """ if not display_name:
                return Response({'error': 'Nom complet requis.'}, status=400) """
            ...
            user = Users.objects.create(
                ...
                homme_femme=request.data.get('homme_femme'),
                must_change_password=False,  # L'utilisateur a choisi son mot de passe
                is_active=True,
            )
            ...
            Notifications.objects.create(
                destinataire=user,
                type='bienvenue',
                titre='Bienvenue sur Klassa',
                contenu=f'Bienvenue {user.display_name} ! Votre compte a été créé avec succès.',
                lu=False
            )
            
            # Log d'activité (pas de created_by car inscription publique)
            LogsActivite.objects.create(
                user=user,
                action='self_register',
                table_cible='Users',
                id_cible=user.id,
                details_json={'role': role, 'source': 'public_register'}
            )
...
class ClassViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'eleve':
            # ✅ Mode recherche de classes disponibles pour inscription
            disponibles = self.request.query_params.get('disponibles')

            if disponibles == 'true':
                return qs.filter(statut='active')

            # Sinon : classes déjà liées à l'élève
    ...
    def perform_create(self, serializer):
        # 🔥 Le nom est maintenant obligatoire et fourni par le frontend
        ...
        # Si aucun professeur n'est envoyé et que l'utilisateur connecté est professeur,
        # on lui attribue la classe automatiquement.
        if not professeur and self.request.user.role == 'professeur':
        ...
        details_cours = serializer.validated_data.get('details_cours')
        statut = serializer.validated_data.get('statut') or 'active'
        ...
        # 🔥 générer jitsi_room_id seulement s'il n'existe pas
...
class InscriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        # Lecture + création + modification des inscriptions
        if self.action in ['list', 'retrieve', 'create', 'update', 'partial_update']:
            return [permissions.IsAuthenticated()]

        # Suppression réservée à la permission existante
        return [permissions.IsAuthenticated(), CanManageInscriptions()]
    

    def get_queryset(self):
        """
        Filtrage par classe via ?classe={id}
        Filtrage par élève via ?eleve_id={id}
        Filtrage par statut via ?statut={statut}
        """
        user = self.request.user
        ...
        # ✅ Un professeur ne voit que les inscriptions de ses classes
        if user.role == 'professeur':
            qs = qs.filter(classe__professeur=user)
        ...
        if eleve_id and user.role != 'eleve':
        ...
        professeur_id = self.request.query_params.get('professeur_id')
        if professeur_id:
            qs = qs.filter(classe__professeur_id=professeur_id)

        return qs.select_related('eleve', 'classe')
    ...
    def perform_create(self, serializer):
        ...
        # ✅ Si c'est l'élève lui-même qui demande l'inscription
        if user.role == 'eleve':
            eleve = user

        if not eleve:
            raise ValidationError({
                "eleve": "L'élève est obligatoire."
            })
        ...
        eleve_nom = inscription.eleve.display_name

        if statut == 'active':
            msg_prof = f"Inscription de l'élève {eleve_nom} à la classe {classe_nom}"
            msg_eleve = f"Vous êtes inscrit(e) à la classe {classe_nom}"
        else:
            msg_prof = f"Demande d'inscription de {eleve_nom} à la classe {classe_nom}"
            msg_eleve = f"Votre demande d'inscription à la classe {classe_nom} a été envoyée"

        Notifications.objects.create(
            destinataire=inscription.classe.professeur,
            type='inscription_eleve',
    ...
    def perform_update(self, serializer):
        ...
        # ❌ Un élève ne peut pas modifier une inscription
        if user.role == 'eleve':
            raise PermissionDenied(
                "Les élèves ne peuvent pas modifier une inscription."
            )

        # ✅ Un professeur ne peut modifier que les inscriptions de ses classes
        if user.role == 'professeur' and inscription.classe.professeur_id != user.id:
            raise PermissionDenied(
                "Vous ne pouvez modifier que les inscriptions de vos classes."
            )
